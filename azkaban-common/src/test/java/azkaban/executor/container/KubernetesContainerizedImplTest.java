...
public class KubernetesContainerizedImplTest {
  ...
  private static final FlowRecommendation DEFAULT_FLOW_RECOMMENDATION =
      new FlowRecommendation(1, 1, "flow", null, null, null);
  ...
  public void testCPUAndMemoryRequestedFromVPARecommender() throws Exception {
    ...
    // flow recommendation cannot be less than min allowed resource limits
    final FlowRecommendation tooSmallFlowRecommendation = new FlowRecommendation(1, 1, "flow",
        "1m", "1Mi", null);
    Assert.assertTrue(this.kubernetesContainerizedImpl.getFlowContainerCPURequest(emptyFlowParam,
        tooSmallFlowRecommendation.getCpuRecommendation()).equals(MIN_ALLOWED_CPU));
    Assert.assertTrue(this.kubernetesContainerizedImpl.getFlowContainerMemoryRequest(emptyFlowParam,
        tooSmallFlowRecommendation.getMemoryRecommendation()).equals(MIN_ALLOWED_MEMORY));

    // flow recommendation cannot be greater than max allowed resource limits
    final FlowRecommendation tooLargeFlowRecommendation = new FlowRecommendation(1, 1, "flow",
        "20", "1Ti", null);
    Assert.assertTrue(this.kubernetesContainerizedImpl.getFlowContainerCPURequest(emptyFlowParam,
        tooLargeFlowRecommendation.getCpuRecommendation()).equals(MAX_ALLOWED_CPU));
    Assert.assertTrue(this.kubernetesContainerizedImpl.getFlowContainerMemoryRequest(emptyFlowParam,
        tooLargeFlowRecommendation.getMemoryRecommendation()).equals(MAX_ALLOWED_MEMORY));

    // max of flow recommendation and flow param will be taken
    final FlowRecommendation lessThanFlowParamFlowRecommendation = new FlowRecommendation(1, 1,
        "flow", "2", "5Gi", null);
    Assert.assertTrue(this.kubernetesContainerizedImpl.getFlowContainerCPURequest(defaultFlowParam,
        lessThanFlowParamFlowRecommendation.getCpuRecommendation()).equals(cpuRequestedInFlowParam));
    Assert.assertTrue(this.kubernetesContainerizedImpl.getFlowContainerMemoryRequest(defaultFlowParam,
        lessThanFlowParamFlowRecommendation.getMemoryRecommendation()).equals(memoryRequestedInFlowParam));

    // max of flow recommendation and flow param will be taken
    final FlowRecommendation greaterThanFlowParamFlowRecommendation = new FlowRecommendation(1, 1,
        "flow", "4", "7Gi", null);
    Assert.assertTrue(this.kubernetesContainerizedImpl.getFlowContainerCPURequest(defaultFlowParam,
        greaterThanFlowParamFlowRecommendation.getCpuRecommendation()).equals(greaterThanFlowParamFlowRecommendation.getCpuRecommendation()));
    Assert.assertTrue(this.kubernetesContainerizedImpl.getFlowContainerMemoryRequest(defaultFlowParam,
        greaterThanFlowParamFlowRecommendation.getMemoryRecommendation()).equals(greaterThanFlowParamFlowRecommendation.getMemoryRecommendation()));
